...
**Student Model:**  
The student is a smaller ResNet (e.g., **ResNet-18**) modified for knowledge distillation by attaching an additional branch to its second residual block. This branch consists of two linear layers with a ReLU activation in between. The student produces:
...
- **Teacher:** A large ResNet (e.g., ResNet-152) that produces both final predictions and intermediate features.
...
This project reproduces some computations from the paper [A Mean Field View of the Landscape of Two-Layers Neural Networks](https://arxiv.org/abs/1804.06561) by Song Mei, Andrea Montanari, and Phan-Minh Nguyen.
...
- \(\sigma: \mathbb{R}^D \to \mathbb{R}\) is the activation function,
- \(\theta_i = (a_i, w_i, b_i)\) represents the parameters for neuron \( i \).
...
As \(N \to \infty\) and the step size becomes small, this distribution converges (in a weak sense) to a continuum distribution \(\rho_t\) that satisfies the following **distributional dynamics (DD) PDE**:
$$
...
- \( I_t \) is the temporal derivative,
...
- **Evaluation:** We demonstrate the network’s performance by showing sample images of the computed flow, the PINN-predicted flow, and the difference between the two.
